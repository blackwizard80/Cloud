...
// Call signal එක listen කිරීම සහ notification පාලනය
onValue(ref(db, 'calls/global_call_signal'), async (snapshot) => {
    const data = snapshot.val();

    // 1. Call එකක් නැතිනම් හෝ අනිත් කෙනා cut කර ඇත්නම් (data null නම්)
    if (!data) {
        incomingAlert.style.display = 'none'; // Notification එක වහාම වසන්න
        
        // ඔබ දැනටමත් video call එක ඇතුළේ සිටී නම් එයද වසා දමන්න
        if (typeof videoModal !== 'undefined' && videoModal.style.display === "flex") {
            hangupCall(); 
        }
        return; 
    }

    // 2. ඔබ දැනටමත් Login වී ඇත්නම් පමණක් (sender නමක් තිබේ නම්) notification පෙන්වන්න
    // මෙය මගින් Login Screen එකේදී notification පෙනීම වළක්වයි
    if (sender && data.offer && data.status === "calling" && sender !== data.callerName) {
        incomingAlert.style.display = 'flex';
        
        // Caller ගේ නම notification එකේ පෙන්වීමට
        ...
        // ඔබ login වී නැතිනම් හෝ වෙනත් හේතුවක් නිසා notification එක වසා තබන්න
...
acceptCallBtn.addEventListener("click", async () => {
    ...
    // Get Caller's existing candidates
...
// මයික් එක On/Off කිරීමේ ක්‍රියාවලිය
micToggleBtn.addEventListener("click", () => {
    if (localStream) {
        isMicMuted = !isMicMuted;
        
        // Audio track එක disable/enable කිරීම
        localStream.getAudioTracks().forEach(track => {
            track.enabled = !isMicMuted;
        });

        // අයිකන් එක සහ බටන් එකේ පාට වෙනස් කිරීම
...
// 4. Hangup
// 4. Hangup සහ Decline කිරීමේ ක්‍රියාවලිය
hangupBtn.addEventListener("click", hangupCall);

async function hangupCall() {
    // Firebase එකේ call එක ඉවර වුණා කියලා update කරනවා
    try {
        ...
        console.log("Call cleanup error:", err);
    }
    ...
    // සම්පූර්ණයෙන්ම පිරිසිදු වීමට page එක refresh කරන්න (Optional)
    // window.location.reload(); 
}

// Decline button එකට event listener එකක් එකතු කිරීම
document.querySelector(".reject-btn").addEventListener("click", async () => {
    try {
        const { ref, remove } = await import("https://www.gstatic.com/firebasejs/11.1.0/firebase-database.js");
        // Firebase එකෙන් call එක අයින් කරනවා
        await remove(ref(db, 'calls/global_call_signal'));
        
        // Alert එක screen එකෙන් අයින් කරනවා
